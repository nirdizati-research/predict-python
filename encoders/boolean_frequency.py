...
def boolean(log: list, event_names: list, label: LabelContainer, encoding: EncodingContainer) -> DataFrame:
    return _encode_boolean_frequency(log, event_names, label, encoding)


def frequency(log: list, event_names: list, label: LabelContainer, encoding: EncodingContainer) -> DataFrame:
    return _encode_boolean_frequency(log, event_names, label, encoding)


def _encode_boolean_frequency(log: list, event_names: list, label: LabelContainer,
                              encoding: EncodingContainer) -> DataFrame:
...
def _create_event_happened(event_names: list, encoding: EncodingContainer) -> list:
...
def _update_event_happened(event, event_names: list, event_happened: list, encoding: EncodingContainer) -> None:
...
def _create_columns(event_names: list, label: LabelContainer) -> list:
    columns = ["trace_id"]
    columns = list(np.append(columns, event_names).tolist())
    return compute_label_columns(columns, label)


def _trace_to_row(trace: Trace, encoding: EncodingContainer, event_index: int, label=None, executed_events=None,
