...
def encode_logs(training_log: list, test_log: list, encoding_type: str, job_type: str, prefix_length=1,
    ...
    training_df = encode_log(training_log, encoding_type, job_type, prefix_length, event_names,
                             zero_padding=zero_padding, add_elapsed_time=add_elapsed_time)
...
def encode_log(run_log: list, encoding_type: str, job_type: str, prefix_length=1, event_names=None, add_label=True,
    ...
    if encoding_type == SIMPLE_INDEX:
        ...
        run_df = simple_index(run_log, event_names, prefix_length=prefix_length, next_activity=next_activity,
                              add_label=add_label, zero_padding=zero_padding, add_elapsed_time=add_elapsed_time)
    elif encoding_type == BOOLEAN:
        run_df = boolean(run_log, event_names, add_label=add_label, add_elapsed_time=add_elapsed_time)
    elif encoding_type == FREQUENCY:
        run_df = frequency(run_log, event_names, add_label=add_label, add_elapsed_time=add_elapsed_time)
