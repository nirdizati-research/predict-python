...
class EncodingParser:
    ...
    def _parse_train_data_complex(self, train_data: DataFrame) -> ndarray:
        ...
        the parsing is similar to simple_index, but for time series prediction some reshaping and column dropping is needed
        :param train_data: input dataset
        :return: parsed dataset
        """
        if self._task == Tasks.TIME_SERIES_PREDICTION:
    ...
    def _parse_test_data_complex(self, test_data: DataFrame) -> ndarray:
        ...
        same steps as simple_encoding parsing, but with the addition of reshaping and column dropping in the time series prediction case
        :param test_data: input dataset
        :return: parsed dataset
        """
        if self._task == Tasks.TIME_SERIES_PREDICTION:
