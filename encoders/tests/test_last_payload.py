...
class LastPayload(TestCase):
    ...
    def test_shape(self):
        df = last_payload(self.log, self.label, self.add_col, prefix_length=2)
    ...
    def test_prefix1(self):
        df = last_payload(self.log, self.label, self.add_col, prefix_length=1)
    ...
    def test_prefix1_no_label(self):
        df = last_payload(self.log, LabelContainer(NO_LABEL), self.add_col, prefix_length=1)
    ...
    def test_prefix1_no_elapsed_time(self):
        df = last_payload(self.log, LabelContainer(add_elapsed_time=False), self.add_col, prefix_length=1)
    ...
    def test_prefix5(self):
        df = last_payload(self.log, self.label, self.add_col, prefix_length=5)

        self.assertEqual(df.shape, (2, 12))

    def test_prefix10(self):
        df = last_payload(self.log, self.label, self.add_col, prefix_length=10)

        self.assertEqual(df.shape, (1, 17))

    def test_prefix10_zero_padding(self):
        df = last_payload(self.log, self.label, self.add_col, prefix_length=10, zero_padding=True)
