...
def explain(cffeedback_exp: Explanation, training_df, test_df, top_k):
    ...
    split_obj = Split.objects.filter(pk=job_obj.split_id)[0]
    ...
    # RETRIEVE&SAVE TS
    ts_exp_job, _ = Explanation.objects.get_or_create(type=ExplanationTypes.TEMPORAL_STABILITY.value, split=split_obj,
                                                      predictive_model=job_obj.predictive_model, job=job_obj)
    ts = temporal_stability(ts_exp_job, training_df, test_df1, explanation_target=None)

    # RETRIEVE&SAVE LIMETS
    limets_exp_job, _ = Explanation.objects.get_or_create(type=ExplanationTypes.LIME.value, split=split_obj,
                                                          predictive_model=job_obj.predictive_model, job=job_obj)
            ...
            key: [
                ...
                for element in sorted(
                    [(pref, lime_ts[key]['prefix_' + str(job_obj.encoding.prefix_length)][pref]['value'],
                      lime_ts[key]['prefix_' + str(job_obj.encoding.prefix_length)][pref]['importance']) for pref in
                     lime_ts[key]['prefix_' + str(job_obj.encoding.prefix_length)]],
    ...
    return {"confusion_matrix": confusion_matrix, "data": data,
            "freq_seqs_after_filter": freq_seqs_after_filter, "filtered_freq_seqs_after_filter": filtered_freq_seqs_after_filter}
